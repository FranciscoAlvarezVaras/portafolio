...
const columns: Column[] = [
  ...
  { key: 'storage', label: { es: 'Lake · bronze', en: 'Lake · bronze' }, x: C[2] },
  { key: 'transform', label: { es: 'Warehouse · silver & gold', en: 'Warehouse · silver & gold' }, x: C[3] },
  { key: 'consume', label: { es: 'Consumo', en: 'Consumption' }, x: C[4] },
];
...
const nodes: PipelineNode[] = [
  ...
  },
  {
    ...
    evidence: [ev.tfm({ es: 'Catálogo de tablas, linaje y etiquetas PII en Dataplex', en: 'Table catalogue, lineage and PII tags in Dataplex' })],
...
/* Flujo de datos ------------------------------------------------------ */
const edges: Edge[] = [
  { from: 'src_db', to: 'ing_batch', label: { es: 'SQL', en: 'SQL' } },
  { from: 'src_api', to: 'ing_batch', label: { es: 'JSON', en: 'JSON' } },
  { from: 'src_files', to: 'ing_batch', label: { es: 'CSV', en: 'CSV' } },
  { from: 'src_events', to: 'ing_stream', label: { es: 'eventos', en: 'events' } },
  { from: 'ing_batch', to: 'lake', label: { es: 'carga diaria', en: 'daily load' } },
  { from: 'ing_stream', to: 'lake', label: { es: 'en continuo', en: 'continuous' } },
  { from: 'lake', to: 'silver', label: { es: 'crudo', en: 'raw' } },
...
export const reference: Architecture = {
  id: 'reference',
  name: { es: 'Referencia', en: 'Reference' },
  tagline: { es: 'Válida en cualquier nube', en: 'Works on any cloud' },
  intro: {
    es: 'La versión sin marcas: las mismas capas y las mismas reglas sirvan cual sea la nube. Es el esquema mental que aplico antes de elegir servicios.',
    en: 'The brand-free version: the same layers and rules apply whatever the cloud. It is the mental model I apply before picking services.',
  },
  columns,
  nodes,
  edges,
  canvas: { width: C[4] + W + 24, height: 612 },
  layers: [
    {
      key: 'sources',
      title: { es: 'Fuentes', en: 'Sources' },
      what: { es: 'Donde nacen los datos: sistemas internos, APIs externas, eventos y ficheros.', en: 'Where data is born: internal systems, external APIs, events and files.' },
      why: { es: 'Cada tipo de fuente falla de una manera distinta y pide una ingesta distinta.', en: 'Each kind of source fails differently and needs a different ingestion method.' },
      services: ['PostgreSQL', 'REST', 'Pub/Sub', 'Excel'],
      mine: { es: 'RRHH y Slack en el TFM; GPS, clima y aire en CloudRISK; catálogos públicos en el monitor de precios.', en: 'HR and Slack in the thesis; GPS, weather and air in CloudRISK; public catalogues in the price monitor.' },
    },
    {
      key: 'ingest',
      title: { es: 'Ingesta', en: 'Ingestion' },
      what: { es: 'Mover datos sin transformarlos: batch a hora fija o streaming en continuo.', en: 'Move data without transforming it: batch on a schedule or continuous streaming.' },
      why: { es: 'Separar extraer de transformar permite reintentar sin perder nada.', en: 'Separating extraction from transformation lets you retry without losing anything.' },
      services: ['Airflow', 'Cloud Run Jobs', 'Pub/Sub', 'Dataflow'],
      mine: { es: 'CronJob diario del monitor de precios y pipeline stateful de Beam en CloudRISK.', en: 'Daily CronJob for the price monitor and a stateful Beam pipeline in CloudRISK.' },
    },
    {
      key: 'storage',
      title: { es: 'Lake · bronze', en: 'Lake · bronze' },
      what: { es: 'El dato tal cual llegó, inmutable y particionado por fecha.', en: 'Data as it arrived, immutable and partitioned by date.' },
      why: { es: 'Es lo que permite reprocesar cuando una transformación sale mal.', en: 'It is what lets you reprocess when a transformation goes wrong.' },
      services: ['Cloud Storage', 'BigQuery'],
      mine: { es: 'Bronze normalizada en el TFM; tabla de snapshots diarios en el monitor de precios.', en: 'Normalised bronze in the thesis; daily snapshot table in the price monitor.' },
    },
    {
      key: 'transform',
      title: { es: 'Warehouse · silver y gold', en: 'Warehouse · silver and gold' },
      what: { es: 'Modelos SQL versionados: limpieza, tests y marts con KPIs definidos.', en: 'Versioned SQL models: cleaning, tests and marts with defined KPIs.' },
      why: { es: 'Aquí se define cada métrica una sola vez, con dueño, fórmula y umbral.', en: 'This is where every metric is defined once, with owner, formula and threshold.' },
      services: ['dbt', 'Dataform', 'BigQuery'],
      mine: { es: 'Vistas gold y catálogo de KPIs de retención; modelos dbt cada cinco minutos en Calidad del aire.', en: 'Gold views and retention KPI catalogue; dbt models every five minutes in Air quality.' },
    },
    {
      key: 'consume',
      title: { es: 'Consumo', en: 'Consumption' },
      what: { es: 'Dashboards, modelos y APIs que leen la misma capa gold.', en: 'Dashboards, models and APIs reading the same gold layer.' },
      why: { es: 'Un solo origen para todos los consumidores: no hay tres versiones del KPI.', en: 'One source for every consumer: there are not three versions of the KPI.' },
      services: ['Looker Studio', 'Vertex AI', 'FastAPI'],
      mine: { es: 'Centro de mando en Looker Studio, dashboard Dash y backend FastAPI de CloudRISK.', en: 'Looker Studio command centre, Dash dashboard and the CloudRISK FastAPI backend.' },
    },
    {
      key: 'cross',
      title: { es: 'Transversal', en: 'Cross-cutting' },
      what: { es: 'Orquestación, gobierno, infraestructura como código y monitorización.', en: 'Orchestration, governance, infrastructure as code and monitoring.' },
      why: { es: 'No mueven datos, pero sin ellas el pipeline no sobrevive al primer mes.', en: 'They move no data, but without them the pipeline does not survive its first month.' },
      services: ['Airflow', 'Dataplex', 'Terraform', 'Cloud Logging'],
      mine: { es: 'Terraform y CI/CD en CloudRISK y el Student Hub; alertas por Telegram en Calidad del aire.', en: 'Terraform and CI/CD in CloudRISK and the Student Hub; Telegram alerts in Air quality.' },
    },
  ],
  story: [
    { node: 'src_db', title: { es: 'Nace un registro', en: 'A record is born' }, text: { es: 'RRHH actualiza la ficha de un empleado en su sistema interno.', en: 'HR updates an employee record in the internal system.' } },
    { node: 'ing_batch', title: { es: 'Se extrae', en: 'It gets extracted' }, text: { es: 'A las seis de la mañana un job lee la tabla y deposita una copia cruda en el lake.', en: 'At six in the morning a job reads the table and drops a raw copy into the lake.' } },
    { node: 'lake', title: { es: 'Aterriza sin tocar', en: 'It lands untouched' }, text: { es: 'La copia queda particionada por fecha. Si algo sale mal después, se reprocesa desde aquí.', en: 'The copy is partitioned by date. If anything goes wrong later, it is reprocessed from here.' } },
    { node: 'silver', title: { es: 'Se limpia', en: 'It gets cleaned' }, text: { es: 'Un modelo SQL normaliza tipos, elimina duplicados y define qué es "empleado activo".', en: 'A SQL model normalises types, removes duplicates and defines what "active employee" means.' } },
    { node: 'quality', title: { es: 'Pasa los tests', en: 'It passes the tests' }, text: { es: 'Unicidad, nulos y rangos. Si algo falla, el pipeline se para y avisa antes de llegar a gold.', en: 'Uniqueness, nulls and ranges. If anything fails the pipeline stops and alerts before reaching gold.' } },
    { node: 'gold', title: { es: 'Se convierte en KPI', en: 'It becomes a KPI' }, text: { es: 'Entra en la tabla de hechos y actualiza la tasa de rotación con su definición escrita.', en: 'It enters the fact table and updates the churn rate with its written definition.' } },
    { node: 'dashboard', title: { es: 'Alguien decide', en: 'Someone decides' }, text: { es: 'El dashboard pasa el umbral, el manager lo ve y convoca una conversación antes de que sea tarde.', en: 'The dashboard crosses the threshold, the manager sees it and schedules a conversation before it is too late.' } },
  ],
  scenarios: [
    { node: 'orchestration', label: { es: '¿Y si falla la orquestación?', en: 'What if orchestration fails?' } },
    { node: 'quality', label: { es: '¿Y si no hay tests de calidad?', en: 'What if there are no quality tests?' } },
    { node: 'silver', label: { es: '¿Y si saltamos la capa silver?', en: 'What if we skip the silver layer?' } },
    { node: 'iac', label: { es: '¿Y si no hay Terraform?', en: 'What if there is no Terraform?' } },
  ],
};
